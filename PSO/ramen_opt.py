...
class Environment():
    def __init__(self):
        self.env = []
        for i in range(10):
            self.env.append(gym.make('myenv-v0'))
        print('observation space:', self.env[0].observation_space)
        print('action space:', self.env[0].action_space)

        obs = self.env[0].reset()
        print('initial observation:', obs)

        action = self.env[0].action_space.sample()
        obs, r, done, info = self.env[0].step(action)
    ...
    def Optimize_PSO(self):
        ...
        init_X = np.ones(self.env[0].n_story*2)*4/9
        init_F = self.env[0]._step_for_optimization()
        bounds = np.zeros((self.env[0].n_story*2,2))
        bounds[:,1] = 1
        optimizer = PSO.PSO(init_X,init_F,bounds)

        t1 = time.process_time()
        for cycle in range(10):

            optimizer.Reset()
            for i in range(optimizer.np):
                _ = self.env[i].reset(optimizer.swarm[i].p_i)

            iteration = 0
            while iteration < N_ITERATIONS:
                for i in range(len(optimizer.swarm)):
                    state1 = self.env[i].state["sec"].astype(np.float32).reshape((1,6))
                    state2 = self.env[i].state["sec_bound"].astype(np.float32).reshape((1,4))
                    state3 = self.env[i].state["stress"].astype(np.float32).reshape((1,6))
                    state4 = self.env[i].state["cof"].astype(np.float32).reshape((1,4))
                    state5 = self.env[i].state["fl_bound"].astype(np.float32).reshape((1,2))
                    state = np.concatenate((state1,state2,state3,state4,state5),axis=1)
                    Q = self.agent.brain.model.forward(state).data[0]
                    if np.argmax(Q) == 4:
                        pass
                    else:
                        pass
                        if Q[0] > Q[2]:
                            optimizer.swarm[i].p_i[2*self.env[i].current_floor] += np.random.rand()/9.0
                        else:
                            optimizer.swarm[i].p_i[2*self.env[i].current_floor] -= np.random.rand()/9.0
                        if Q[1] > Q[3]:
                            optimizer.swarm[i].p_i[2*self.env[i].current_floor+1] += np.random.rand()/9.0
                        else:
                            optimizer.swarm[i].p_i[2*self.env[i].current_floor+1] -= np.random.rand()/9.0
                    optimizer.swarm[i].p_i[optimizer.swarm[i].p_i<0] = 0.0
                    optimizer.swarm[i].p_i[optimizer.swarm[i].p_i>1] = 1.0
                    optimizer.swarm[i].Update(optimizer.p_best_g,iteration/N_ITERATIONS)
                    
                for i in range(len(optimizer.swarm)): 
                    self.env[i].set_sec_from_variable(optimizer.swarm[i].p_i)
                    F = self.env[i]._step_for_optimization()
                    iteration += 1
                    if F < optimizer.swarm[i].f_best_i:
                        optimizer.swarm[i].p_best_i = copy.copy(optimizer.swarm[i].p_i)
                        optimizer.swarm[i].f_best_i = F
                    if F < optimizer.f_best_g:
                        optimizer.p_best_g = copy.copy(optimizer.swarm[i].p_i)
                        optimizer.f_best_g = F

                    self.env[i].current_floor += 1
                    if(self.env[i].current_floor == self.env[i].n_story):
                        self.env[i].current_floor -= self.env[i].n_story

                    self.env[i].state_update()

                optimizer.history.append(optimizer.f_best_g)
            ...
            # pick up the best solution
            self.env[0].set_sec_from_variable(optimizer.p_best_g)
            self.env[0]._render(mode='shape',name='opt{0:0=2}_shape'.format(cycle))
            self.env[0]._render(mode='section',name='opt{0:0=2}_section'.format(cycle))
            self.env[0]._render(mode='ratio',name='opt{0:0=2}_ratio'.format(cycle))
